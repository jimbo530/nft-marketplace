...
/// @title NFT Power-Up (Elven) — WETH → EGP/PR25 LP → NFT
/// @notice Buys EGP + PR25, creates LP, sends to NFT contract to boost backing
contract PowerUpElven {
    ...
    /// @notice Spend WETH to create EGP/PR25 LP and send it to an NFT contract
    ...
    function _addLpAndSend(address to) internal returns (uint liquidity) {
        ...
        (,, liquidity) = router.addLiquidity(EGP, PR25, balEgp, balPr25, 0, 0, to, block.timestamp + 300);
    }
