...
/// @title MfT NFT Primary Marketplace (Polygon) — WETH payments, tokenA/PR25 LP building
/// @notice Sells ERC-1155 NFTs. Payment splits: 50% LP->NFT, 20% artist, 30% LP->community. No platform fee.
contract NFTMarketplacePoly {
    ...
    constructor(address _tokenA, address _communityWallet) {
        tokenA = _tokenA;
        owner = msg.sender;
    ...
    function totalCost(uint listingId) external view returns (uint) {
        return listings[listingId].priceWeth;
    }
    ...
    function buy(uint listingId) external {
        ...
        IERC20(WETH).transferFrom(msg.sender, address(this), l.priceWeth);

        uint forNftLp = (l.priceWeth * 50) / 100;
        uint forArtist = (l.priceWeth * 20) / 100;
        uint forCommunityLp = l.priceWeth - forNftLp - forArtist;
        ...
        emit Sold(listingId, msg.sender, forNftLp, forCommunityLp, forArtist);
    }
    ...
    function _makeLP(uint wethAmount, address to) internal returns (uint liquidity) {
        ...
        path1[1] = PR25;
        path1[2] = tokenA;
        router.swapExactTokensForTokens(half, 0, path1, address(this), block.timestamp + 300);
        ...
        uint bal1 = IERC20(tokenA).balanceOf(address(this));
        uint balPr25 = IERC20(PR25).balanceOf(address(this));
        IERC20(tokenA).approve(address(router), bal1);
