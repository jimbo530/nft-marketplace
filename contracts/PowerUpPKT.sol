...
/// @title NFT Power-Up (PKT) — WETH -> PKT/PR25 LP -> NFT
/// @notice Buys PKT + PR25, creates LP, sends to NFT contract to boost backing
contract PowerUpPKT {
    ...
    /// @notice Spend WETH to create PKT/PR25 LP and send it to an NFT contract
    ...
    function _addLpAndSend(address to) internal returns (uint liquidity) {
        ...
        (,, liquidity) = router.addLiquidity(PKT, PR25, balPkt, balPr25, 0, 0, to, block.timestamp + 300);
    }
