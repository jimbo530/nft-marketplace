...
/// @title MfT NFT Resale — Dwarven Fortress (Polygon) — WETH payments, LGP/PR25 LP building
/// @notice 50% to seller, then 50% LP→NFT, 20% artist, 30% LP→community. 1% platform fee on top.
contract NFTResaleLGP_Poly {
    ...
    uint public constant PLATFORM_FEE_BPS = 100;
    ...
    function _makeLP(uint wethAmount, address to) internal returns (uint liquidity) {
        ...
        uint bal1  = IERC20(LGP).balanceOf(address(this));
        uint balPr25 = IERC20(PR25).balanceOf(address(this));
        IERC20(LGP).approve(address(router), bal1);
